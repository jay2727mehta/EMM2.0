...
const MediumHumidCard = ({ Data, chartData }) => {
  ...
  const dataValues = Data && Data.meter_reading ? convertDataValues(Data.meter_reading, humidity) : [];
  ...
  const convertedMinHumid = Data ? convertTemperature(Data.min_humidity, humidity) : null;
  const convertedMaxHumid = Data ? convertTemperature(Data.max_humidity, humidity) : null;
  const convertedCriticalHumid = Data ? convertTemperature(Data.critical_humidity, humidity) : null;

  function getGradient(ctx, chartArea, data) {
    if (!Data || isNaN(convertedMinHumid) || isNaN(convertedMaxHumid) || isNaN(convertedCriticalHumid)) {
    ...
    const colorScale = (value) => {
      if (value >= convertedMinHumid && value <= convertedMaxHumid) {
        return '#76C739'; // Green
      } else if (value > convertedMaxHumid && value < convertedCriticalHumid) {
        return '#e7af84'; // Orange
      } else if (value >= convertedCriticalHumid || value < convertedMinHumid) {
        return '#F26457'; // Red
      }
    };
      ...
      <Box display="flex" alignItems="center" justifyContent="space-evenly" gap='20px'>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'left', color: tempCheck.colors }}>
            <HumidIcon className="temperature-icon" width={32} height={32} />
...
export default MediumHumidCard;
