...
const MediumCurrentCard = ({ Data, chartData }) => {
  // const [power, setpower] = useState('kW');
  // const [anchorEl, setAnchorEl] = useState(null);
  // const { ref } = useFitText();
  ...
  const convertedValue = Data ? convertPower(powerValue, power) : 0;
  const formattedValue = Data ? formatValue(parseInt(convertedValue)) : 0;
  const tempCheck = getColorSettings(powerValue);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2,}}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', }}>
            {Data?.meter_reading?.length ?
              <Box>
                <Box sx={{
                  textAlign: 'center', fontSize: fontSize, fontWeight: 'bold', color: tempCheck.colors, position: 'relative', bottom: 10, whiteSpace: 'nowrap',
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                }}>
                  {formattedValue}
...
export default MediumCurrentCard;
