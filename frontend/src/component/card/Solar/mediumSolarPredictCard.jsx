import React, { useState } from 'react';
import { Box, Typography, Card, CardContent, FormControl, Menu, MenuItem } from '@mui/material';
import ThermostatIcon from '@mui/icons-material/Thermostat';
import { Line } from 'react-chartjs-2';
import { Chart as ChartJS, CategoryScale, LinearScale, PointElement, LineElement, Tooltip, Legend } from 'chart.js';
import { ReactComponent as TemperatureIcon } from "../../config/svgfiles/zap.svg";
import { ReactComponent as WifiOffIcon } from "../../config/svgfiles/wifi-off.svg";
import { useFitText } from '../../config/fontResizeConfig';
import { type } from '@testing-library/user-event/dist/type';

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Tooltip, Legend);

const MediumSolarPredictCard = ({ Data, chartData }) => {
  const [energy, setEnergy] = useState('kW');
  const [anchorEl, setAnchorEl] = useState(null);

  const convertEnergy = (value, unit) => {
    switch (unit) {
      case 'W':
        return value * 1000;
      case 'kW':
      default:
        return value;
    }
  };

  const formatValue = (value) => {
    if (!value) {
      return 0; // Return a default value if `value` is undefined or null
    }

    if (value >= 1_000_000_000) {
      return (value / 1_000_000_000).toFixed(1) + 'b'; // Billions
    } else if (value >= 1_000_000) {
      return (value / 1_000_000).toFixed(1) + 'm'; // Millions
    } else if (value >= 1_000) {
      return (value / 1_000).toFixed(1) + 'k'; // Thousands
    } else {
      return value.toFixed(2); // For values less than 1,000
    }
  };

  const handleClick = (event) => setAnchorEl(event.currentTarget);
  const handleClose = () => setAnchorEl(null);
  const handleMenuItemClick = (value) => {
    setEnergy(value);
    handleClose();
  };

  const convertDataValues = (values, unit) => {
    return values.map(value => convertEnergy(value, unit));
  };

  const labels = Data && Data?.length ? Data.map((item) => item[0]) : [];
  const dataValues = Data && Data?.length ? convertDataValues(Data.map((item) => item[1]), energy) : [];
  const dataValues2 = Data && Data?.length ? convertDataValues(Data.map((item) => item[2]), energy) : [];

  chartData = {
    labels: labels,
    data: dataValues,
    data2: dataValues2,
  };

  const data = {
    labels: chartData ? chartData.labels : '',
    datasets: [
      {
        label: `Solar Power (${energy})`,
        data: chartData ? chartData.data : '',
        borderColor: 'red',
        tension: 0.4,
        borderWidth: 4,
        pointRadius: 0,
        pointHoverRadius: 0,
      },
      {
        label: `Raw Power (${energy})`,
        data: chartData ? chartData.data2 : '',
        borderColor: 'blue',
        tension: 0.4,
        borderWidth: 4,
        pointRadius: 0,
        pointHoverRadius: 0,
      },
    ],
  };

  const options = {
    responsive: true,
    maintainAspectRatio: false,
    interaction: {
      mode: "nearest",
      intersect: false,
    },
    hover: {
      mode: "nearest",
      intersect: false,
    },
    elements: {
      point: {
        radius: 0, // Remove points
      },
      line: {
        tension: 0.4, // Smooth lines
      },
    },
    scales: {
      x: {
        title: {
          display: true,
          text: 'Days',
          font: {
            size: 16,
            weight: 'bold'
          }
        },
        offset: true,
        grid: {
          display: false,
        },
        ticks: {
          font: {
            size: 10,
            weight : 'bold',
            color: '#9e9e9e',
          },
        },
      },
      y: {
        title: {
          display: true,
          text: 'Energy (kWh)',
          font: {
            size: 16,
            weight: 'bold'
          }
        },
        display: true,
        offset: true,
        ticks: {
          font: {
            size: 10,
            weight : 'bold',
            color: '#9e9e9e',
          },
        },
      },
    },
    plugins: {
      legend: {
        display: false,
      },
      tooltip: {
        display: true,
      }
    },

  };

  const getColorSettings = (value) => {
    if (Data?.length) {
      return { bgColor: '#E9EEEF', borderColor: '#E9EEEF', colors: '#006DBC', fontColor: '#878A8B' };
    }
    else {
      return { bgColor: '#E5EBEB', borderColor: '#E5EBEB', colors: '#757676', fontColor: '#757676' };
    }
  };

  const energyValue = (Data && Array.isArray(Data) && Data.length) ? (Data[Data.length - 1].DayGen) : 0;
  const convertedValue = Data ? convertEnergy(energyValue, energy) : 0;
  const formattedValue = Data ? formatValue(convertedValue) : 0;
  const tempCheck = getColorSettings(energyValue);
  const fontSize = formattedValue ? formattedValue.length > 4 ? '50px' : '60px' : '60px';

  const styles = {
    thinBorder: {
      border: "0.5px solid #E5E7EB",
      boxShadow: "0 8px 16px rgba(0, 0, 0, 0.15)",
      backgroundColor: "rgba(255, 255, 255, 0.68)",
    },
  };

  return (
    <Card sx={{ ...styles.thinBorder, width: 1850, height: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 5, borderRadius: '20px', bgcolor: tempCheck.bgColor, borderColor: tempCheck.borderColor, borderWidth: 2 }}>
      <Box display="flex" alignItems="center" flexDirection='column' justifyContent="space-evenly" gap='20px'>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'left', color: tempCheck.colors }}>
            <TemperatureIcon className="temperature-icon" width={32} height={32} />
            <Typography variant='h6' fontWeight='bold' sx={{ marginLeft: '5px' }}>
              Solar Prediction
            </Typography>
          </Box>
        </Box>
        {!Data?.length ? <Box sx={{ display: 'flex', flexDirection: 'column', }}>
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', color: tempCheck.colors, marginTop: '5px', marginBottom: '25px' }}>
            <WifiOffIcon className="temperature-icon" width={72} height={72} />
          </Box>
        </Box> : <Box sx={{ position: 'relative', bottom: fontSize === '50px' ? 0 : 8 }}>
          <Line data={data} options={options} width='1600' height='500' />
        </Box>}

        <Box sx={{ color: tempCheck.fontColor, position: 'relative', bottom: 15, top: fontSize === '50px' ? 1 : '', }}>
          <Typography sx={{
            textAlign: 'center', fontWeight: 'bold',
          }}>{'SEZ Facility Solar Generation'}</Typography>
          <Typography sx={{
            textAlign: 'center', fontWeight: 'bold', whiteSpace: 'nowrap',
            overflow: 'hidden',
            textOverflow: 'ellipsis',
          }}>{'SEZ Campus'}</Typography>
        </Box>
      </Box>
    </Card>
  );
};

export default MediumSolarPredictCard;

