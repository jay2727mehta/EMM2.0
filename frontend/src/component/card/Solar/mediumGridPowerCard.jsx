...
const MediumGridPowerCard = ({ Data, chartData }) => {
  ...
  const dataValues = Data && Data.length ? convertDataValues(Data.map((item) => item.grid_active_power_kw / 1000), energy) : [];
  const CommitedValue = Data && Data.length ? convertDataValues(Data.map(() => 325), energy) : [];
  ...
  const energyValue = (Data && Array.isArray(Data) && Data.length) ? (Data[Data.length - 1].grid_active_power_kw) / 1000 : 0;
    ...
    <Card sx={{ ...styles.thinBorder, width: 1500, height: '275px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 5, borderRadius: '20px', bgcolor: tempCheck.bgColor, borderColor: tempCheck.borderColor, borderWidth: 2 }}>
      <Box display="flex" alignItems="center" justifyContent="space-evenly" gap='20px'>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', }}>
            ...
            <Box ref={ref} sx={{ color: tempCheck.fontColor, position: 'relative', bottom: fontSize === '45px' ? 10 : 6, top: fontSize === '45px' ? 8 : '', }}>
              ...
              <ToolTipBox title={`Current Grid Power, SEZ Campus`} arrow>
                <Typography
                  sx={{
                    textAlign: 'center',
                    fontWeight: 'bold',
                ...
                >
                  {Data?.meter_name?.toUpperCase() || 'Current Grid Power'}
                </Typography>
              </ToolTipBox>
        ...
        <Box sx={{ position: 'relative', bottom: fontSize === '50px' ? 0 : 8 }}>
          <Line data={data} options={options} width='1200' height='220' />
        </Box>
      </Box>
    </Card>
  );
...
export default MediumGridPowerCard;
